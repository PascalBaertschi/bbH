...
if __name__ == '__main__':
    ...
    parser.add_argument('-T', '--tes',      dest='tes', type=str, default="", action='store',choices=["","TauESdm0Up","TauESdm1Up","TauESdm10Up","TauESdm0Down","TauESdm1Down","TauESdm10Down"],
    ...
    for channel in channels:
      ...
      if not args.haddother or args.make:
        for directory in samplelist:
            ...
            if args.make:
                ...
                comb_files = []
                """
                for i in range(11,number_files,10):
                    infiles_loop = infiles[:-5]+"/tree_[%s,%s].root"%(i-10,i)
                    haddcmd_loop = 'hadd -f %s_%s.root %s'%(outfile[:-5],i-10,infiles_loop)
                    print("haddcmd:",haddcmd_loop)
                    os.system(haddcmd_loop)
                    comb_files.append('%s_%s.root'%(outfile[:-5],i-10))
                    if number_files-i<10:
                        infiles_loop = infiles[-5]+"/tree_[%s,%s].root"%(i,number_files)
                        haddcmd_loop = 'hadd -f %s_%s.root %s'%(outfile[:-5],i,infiles_loop)
                        print("haddcmd:",haddcmd_loop)
                        os.system(haddcmd_loop)
                        comb_files.append('%s_%s.root'%(outfile[:-5],i))
                haddcmd_comb = 'hadd -f %s %s'%(outfile,comb_files)
                print("haddcmd_comb:",haddcmd_comb)
                os.system(haddcmd_comb)
                """
                infiles_list = []
                for i in range(1,number_files,1):
                    infiles_loop = infiles[:-5]+"/tree_%s.root"%i
                    infiles_list.append(infiles_loop)
                    if i%10==0:
                        list_string = ' '.join(infiles_list)
                        haddcmd_loop = 'hadd -f %s_%s.root %s'%(outfile[:-5],i-10,list_string)
                        print("haddcmd:",haddcmd_loop)
                        os.system(haddcmd_loop)
                        comb_files.append('%s_%s.root'%(outfile[:-5],i-10))
                        infiles_list = []
      ...
      if args.haddother:
        if year==2016:
            ...
            for sample in list(getsamples("mutau","UL",year,"_comb").values()):
                if "Run2016" in sample:continue #to exclude data samples
                if JECvar=="":
                    ...
                    if tes!="":
                        haddcmd = 'hadd -f %s/samples_UL2016_comb_%s/%s.root %s/samples_UL2016_%s/%s.root %s/samples_UL2016_preVFP_%s/%s.root'%(workdir,tes,sample,workdir,tes,sample,workdir,tes,sample) 
                    else:
                        haddcmd = 'hadd -f %s/samples_UL2016_comb_%s/%s.root %s/samples_UL2016_%s/%s.root %s/samples_UL2016_preVFP_%s/%s.root'%(workdir,JECvar,sample,workdir,JECvar,sample,workdir,JECvar,sample)
                os.system(haddcmd)
