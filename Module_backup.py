...
class Producer(Module):
    ...
    def fillBranches(self,event):
        ...
        self.out.BTagWeight_comb_up[0]      = self.BTagWeight_comb_up
        self.out.BTagWeight_comb_down[0]    = self.BTagWeight_comb_down
        self.out.BTagWeight_jes_up[0]       = self.BTagWeight_jes_up 
        self.out.BTagWeight_jes_down[0]     = self.BTagWeight_jes_down  
        self.out.BTagWeight_lf_up[0]        = self.BTagWeight_lf_up 
        self.out.BTagWeight_lf_down[0]      = self.BTagWeight_lf_down  
        self.out.BTagWeight_hf_up[0]        = self.BTagWeight_hf_up
        self.out.BTagWeight_hf_down[0]      = self.BTagWeight_hf_down 
        self.out.BTagWeight_hfstats1_up[0]  = self.BTagWeight_hfstats1_up 
        self.out.BTagWeight_hfstats1_down[0]= self.BTagWeight_hfstats1_down
        self.out.BTagWeight_hfstats2_up[0]  = self.BTagWeight_hfstats2_up
        self.out.BTagWeight_hfstats2_down[0]= self.BTagWeight_hfstats2_down
        self.out.BTagWeight_lfstats1_up[0]  = self.BTagWeight_lfstats1_up 
        self.out.BTagWeight_lfstats1_down[0]= self.BTagWeight_lfstats1_down  
        self.out.BTagWeight_lfstats2_up[0]  = self.BTagWeight_lfstats2_up 
        self.out.BTagWeight_lfstats2_down[0]= self.BTagWeight_lfstats2_down
        self.out.BTagWeight_cferr1_up[0]    = self.BTagWeight_cferr1_up 
        self.out.BTagWeight_cferr1_down[0]  = self.BTagWeight_cferr1_down  
        self.out.BTagWeight_cferr2_up[0]    = self.BTagWeight_cferr2_up  
        self.out.BTagWeight_cferr2_down[0]  = self.BTagWeight_cferr2_down
    ...
    def analyze(self, event):
        ...
        self.BTagWeight_comb_up    = 1.
        self.BTagWeight_comb_down  = 1.
        self.BTagWeight_jes_up     = 1.
        self.BTagWeight_jes_down   = 1. 
        self.BTagWeight_lf_up      = 1. 
        self.BTagWeight_lf_down    = 1.
        self.BTagWeight_hf_up      = 1.
        self.BTagWeight_hf_down    = 1.
        self.BTagWeight_hfstats1_up   = 1.
        self.BTagWeight_hfstats1_down = 1.
        self.BTagWeight_hfstats2_up   = 1.
        self.BTagWeight_hfstats2_down = 1.
        self.BTagWeight_lfstats1_up   = 1.
        self.BTagWeight_lfstats1_down = 1.
        self.BTagWeight_lfstats2_up   = 1.
        self.BTagWeight_lfstats2_down = 1.
        self.BTagWeight_cferr1_up     = 1.
        self.BTagWeight_cferr1_down   = 1.
        self.BTagWeight_cferr2_up     = 1.
        self.BTagWeight_cferr2_down   = 1.
        self.BTagWeight_shape_up   = 1.
        self.BTagWeight_shape_down = 1.
        self.GenWeight             = 1.
        self.PUWeight              = 1.
        self.MuWeight              = 1.
        self.MuWeight_up           = 1.
        ...
        for i,jet in enumerate(jets):
            if self.JECvar=="":
                ...
                jet_pt = event.Jet_pt_jesTotalDown[i]
                jet_mass = event.Jet_mass_jesTotalDown[i]
            elif self.JECvar=='jesTotalUp':
                jet_pt = event.Jet_pt_jesTotalUp[i]
                jet_mass = event.Jet_mass_jesTotalUp[i]
            if abs(jet.eta) < 2.4 and jet_pt > 20. and jet.jetId >= 2:
                if self.vec(jet).DeltaR(self.vec(pair[0])) > 0.4:
                    if self.vec(jet).DeltaR(self.vec(pair[1])) > 0.4:
                        jet_list.append([jet,jet_pt,jet_mass])
                        if jet.btagDeepFlavB > wp_tight:
                            jet_btag_tight_list.append([jet,jet_pt,jet_mass])
                        if jet.btagDeepFlavB > wp_medium:
                            jet_btag_medium_list.append([jet,jet_pt,jet_mass])
                        if jet.btagDeepFlavB > wp_loose:
                            jet_btag_loose_list.append([jet,jet_pt,jet_mass])
                        # make exclusive lists
                        if wp_medium > jet.btagDeepFlavB > wp_loose:
                            jet_btag_loose_excl_list.append([jet,jet_pt,jet_mass])
                        elif wp_tight > jet.btagDeepFlavB > wp_medium:
                            jet_btag_medium_excl_list.append([jet,jet_pt,jet_mass])
            elif abs(jet.eta) > 2.4 and jet_pt > 20. and jet.jetId >=2:
                if self.vec(jet).DeltaR(self.vec(pair[0])) > 0.4 and self.vec(jet).DeltaR(self.vec(pair[1])) > 0.4:
                    jet_forward_list.append([jet,jet_pt,jet_mass])
            if abs(jet.eta) < 2.4 and jet_pt > 30. and jet.jetId >= 2:   #has to be if!
                if self.vec(jet).DeltaR(self.vec(pair[0])) > 0.4:
                    if self.vec(jet).DeltaR(self.vec(pair[1])) > 0.4:
                        jet_list_tcut.append([jet,jet_pt,jet_mass])
                        if jet.btagDeepFlavB > wp_tight:
                            jet_btag_tight_list_tcut.append([jet,jet_pt,jet_mass])
                        if jet.btagDeepFlavB > wp_medium:
                            jet_btag_medium_list_tcut.append([jet,jet_pt,jet_mass])
                        if jet.btagDeepFlavB > wp_loose:
                            jet_btag_loose_list_tcut.append([jet,jet_pt,jet_mass])
        
        if len(jet_forward_list)>1.:
            self.DEta_jets_forward = abs(jet_forward_list[0][0].eta-jet_forward_list[1][0].eta)
        ...
        if self.isMC:
            ...
            self.BTagWeight = self.btagTool.getWeight(jet_list,'shape','central')
            self.BTagWeight_comb  = self.btagTool.getWeight(jet_list,'comb_incl','central')
            self.BTagWeight_comb_up  = self.btagTool.getWeight(jet_list,'comb_incl','up')
            self.BTagWeight_comb_down  = self.btagTool.getWeight(jet_list,'comb_incl','down')
            self.BTagWeight_jes_up  = self.btagTool.getWeight(jet_list,'shape','up_jes')
            self.BTagWeight_jes_down  = self.btagTool.getWeight(jet_list,'shape','down_jes')
            self.BTagWeight_lf_up  = self.btagTool.getWeight(jet_list,'shape','up_lf')
            self.BTagWeight_lf_down  = self.btagTool.getWeight(jet_list,'shape','down_lf')
            self.BTagWeight_hf_up  = self.btagTool.getWeight(jet_list,'shape','up_hf')
            self.BTagWeight_hf_down  = self.btagTool.getWeight(jet_list,'shape','down_hf')
            self.BTagWeight_hfstats1_up  = self.btagTool.getWeight(jet_list,'shape','up_hfstats1')
            self.BTagWeight_hfstats1_down  = self.btagTool.getWeight(jet_list,'shape','down_hfstats1')
            self.BTagWeight_hfstats2_up  = self.btagTool.getWeight(jet_list,'shape','up_hfstats2')
            self.BTagWeight_hfstats2_down  = self.btagTool.getWeight(jet_list,'shape','down_hfstats2')
            self.BTagWeight_lfstats1_up  = self.btagTool.getWeight(jet_list,'shape','up_lfstats1')
            self.BTagWeight_lfstats1_down  = self.btagTool.getWeight(jet_list,'shape','down_lfstats1')
            self.BTagWeight_lfstats2_up  = self.btagTool.getWeight(jet_list,'shape','up_lfstats2')
            self.BTagWeight_lfstats2_down  = self.btagTool.getWeight(jet_list,'shape','down_lfstats2')
            self.BTagWeight_cferr1_up  = self.btagTool.getWeight(jet_list,'shape','up_cferr1')
            self.BTagWeight_cferr1_down  = self.btagTool.getWeight(jet_list,'shape','down_cferr1')
            self.BTagWeight_cferr2_up  = self.btagTool.getWeight(jet_list,'shape','up_cferr2')
            self.BTagWeight_cferr2_down  = self.btagTool.getWeight(jet_list,'shape','down_cferr2')
        ...
        jet_list.sort(key=lambda x: x[0].btagDeepFlavB,reverse=True)

        
        if len(jet_list) > 0:
            jet1_vars = self.jet_vars(jet_list,0)
            self.Jet1_pt = jet1_vars[0]
            self.Jet1_eta = jet1_vars[1]
            self.Jet1_phi = jet1_vars[2]
            self.Jet1_mass = jet1_vars[3]
            self.Jet1_btag = jet1_vars[4]
            jet1_tlv = self.tlv_jet(jet_list,0)
        if len(jet_list) > 1:
            jet2_vars = self.jet_vars(jet_list,1)
            self.Jet2_pt = jet2_vars[0]
            self.Jet2_eta = jet2_vars[1]
            self.Jet2_phi = jet2_vars[2]
            self.Jet2_mass = jet2_vars[3]
            self.Jet2_btag = jet2_vars[4]
            jet2_tlv = self.tlv_jet(jet_list,1)
        ...
        if len(jet_list) > 2:
            jet3_vars = self.jet_vars(jet_list,2)
            self.Jet3_pt = jet3_vars[0]
            self.Jet3_eta = jet3_vars[1]
            self.Jet3_phi = jet3_vars[2]
            self.Jet3_mass = jet3_vars[3]
            self.Jet3_btag = jet3_vars[4]   

        

        if len(jet_btag_medium_list) > 0:
            bjet1_vars = self.jet_vars(jet_btag_medium_list,0)
            self.Bjet1_pt = bjet1_vars[0]
            self.Bjet1_eta = bjet1_vars[1]
            self.Bjet1_phi = bjet1_vars[2]
            self.Bjet1_mass = bjet1_vars[3]
            Bjet1_tlv = self.tlv_jet(jet_btag_medium_list,0)
        if len(jet_btag_medium_list) > 1:
            bjet2_vars = self.jet_vars(jet_btag_medium_list,1)
            self.Bjet2_pt = bjet2_vars[0]
            self.Bjet2_eta = bjet2_vars[1]
            self.Bjet2_phi = bjet2_vars[2]
            self.Bjet2_mass = bjet2_vars[3]
            Bjet2_tlv = self.tlv_jet(jet_btag_medium_list,1)
        ...
        if len(jet_btag_medium_list)>0 and len(jet_btag_loose_excl_list)>0:
            Bjet_m_tlv = self.tlv_jet(jet_btag_medium_list,0)
            Bjet_l_tlv = self.tlv_jet(jet_btag_loose_excl_list,0)
            self.DRBjets_lm = Bjet_m_tlv.DeltaR(Bjet_l_tlv)
        if len(jet_btag_tight_list)>0 and len(jet_btag_medium_excl_list)>0:
            Bjet_t_tlv = self.tlv_jet(jet_btag_tight_list,0)
            Bjet_m_tlv = self.tlv_jet(jet_btag_medium_excl_list,0)
        ...
        if self.isMC:
            ...
            if "WJets" in self.sample_name and "amcatnloFXFX" in self.sample_name:
                self.wt_w = self.MCCorrection.getWeight_w(isMuTau,isETau)
        ...
        if isMuTau or isETau:
            ...
            self.met_var_qcd = self.MET*cos(tau_tlv.DeltaPhi(met_tlv))/pair[1].pt
        ...
        if isMuTau:
            ...
            self.wt_ffcorr_hmass_tt = ffweightcorr_hmass[1]
            ...
            self.wt_ffcorr_jetpt_tt = ffweightcorr_jetpt[1]
            ffweightcorr_collinearmass = self.ff.ffweightcorr_collinearmass(self.collinear_mass,self.Tau1_decaymode,"mutau")
            self.wt_ffcorr_collinearmass_qcd = ffweightcorr_collinearmass[0]
            self.wt_ffcorr_collinearmass_tt = ffweightcorr_collinearmass[1]
            ffweightcorr_taujmass = self.ff.ffweightcorr_taujmass(self.TauJ_mass,self.Tau1_decaymode,"mutau")
            self.wt_ffcorr_taujmass_qcd = ffweightcorr_taujmass[0]
            self.wt_ffcorr_taujmass_tt = ffweightcorr_taujmass[1]
            ...
            self.wt_ffcorr_jetpt_qcd = ffweightcorr_jetpt[0]
            self.wt_ffcorr_jetpt_tt = ffweightcorr_jetpt[1]
            ffweightcorr_collinearmass = self.ff.ffweightcorr_collinearmass(self.collinear_mass,self.Tau1_decaymode,"etau")
            self.wt_ffcorr_collinearmass_qcd = ffweightcorr_collinearmass[0]
            self.wt_ffcorr_collinearmass_tt = ffweightcorr_collinearmass[1]
            ffweightcorr_taujmass = self.ff.ffweightcorr_taujmass(self.TauJ_mass,self.Tau1_decaymode,"etau")
            self.wt_ffcorr_taujmass_qcd = ffweightcorr_taujmass[0]
            self.wt_ffcorr_taujmass_tt = ffweightcorr_taujmass[1]
