...
from skim_submit import args, bcolors, createJobs, getFileListLocal, submitJobs, split_seq, saveFileListLocal
import itertools
import subprocess
...
def main():
    ...
    for year in years:
      ...
      samplelist = [ ]
      if JECvar=="yes":
          outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples_skim/UL%s%s_JEC/'%(year,preVFP)) 
      elif tes=="yes":
          outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples_skim/UL%s%s_TES/'%(year,preVFP))
      ...
      for channel in channels:
        ...
        for directory in samplelist:
            if JECvar=="yes":
                outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples_skim/UL%s%s_JEC/%s'%(year,preVFP,directory)) 
            elif tes=="yes":
                outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples_skim/UL%s%s_TES/%s'%(year,preVFP,directory)) 

            if directory.find("Run")>0: #run skim only for MC samples
            ...
            # FILE LIST
            infiles = [ ]
            infiles = getFileListLocal(year,directory,preVFP)
            ...
            if JECvar=="yes":
                jobList      = 'joblist_skim/joblistJECvar_%s%s%s_%s_%s%s.txt'%(ULtag,year,preVFP,directory,channel,tag)
            elif tes=="yes":
                jobList      = 'joblist_skim/joblistTES_%s%s%s_%s_%s%s.txt'%(ULtag,year,preVFP,directory,channel,tag)
            with open(jobList, 'w') as jobslog:
              for filename in outfilelist:
                  ...
                  if not file.IsZombie() and file.GetListOfKeys().Contains('Events'):
                    continue
                  infiles = infilelists[chunk]
                  createJobs(jobslog,infiles,outdir,directory,chunk,channel,"UL",preVFP,year,JECvar,tes)
              ...
              if len(misschunks)>0:
                ...
                for chunk in misschunks:
                  infiles = infilelists[chunk]
                  createJobs(jobslog,infiles,outdir,directory,chunk,channel,ULtag,preVFP,year,JECvar,tes)
