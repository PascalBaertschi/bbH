...
from ROOT import TMath,TFile, TH1
path = "/work/pbaertsc/bbh/NanoTreeProducer/CorrectionTools/leptonEfficiencies/TauPOG/"
...
class TauTriggerSFs(object):
    ...
    def __init__(self, year, trigger, wp='Medium', id='DeepTau2017v2p1'):
        """Load tau trigger histograms from files."""
        print("Loading %s trigger SFs for  %s WP of %s ID for year %s..."%(trigger,wp,id,year))
        
        # CHECKS
        dms      = [0,1,10,11]
        triggers = ['ditau','mutau','etau']
        years    = ['2016','2017','2018','2016ULpreVFP','2016ULpostVFP','2017UL','2018UL']
        ids      = ['DeepTau2017v2p1']
        ...
        # GET DATA
        file = ensureTFile(path+'%s_tauTriggerEff_%s.root'%(year,id), 'r')
        ...
        for dm in dms:
          for histtype, histdict in [('data',hists_data),('mc',hists_mc),('sf',hists_sf)]:
            ...
            histdict[dm] = extractTH1(file,histname)
        file.Close()
