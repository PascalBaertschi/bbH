...
def main():
    ...
    for year in years:
      ...
      for file in files:
        if os.path.isfile(outdir+file+".root"):
          rootfile = TFile(outdir+file+".root",'READ')
          if rootfile.IsZombie() or not rootfile.GetListOfKeys().Contains('tree'):
          ...
          createJobs(jobs,file,outdir,name,nChunks,year,path)
          nChunks = nChunks+1
      print("resubmit for",missing_files)
      jobs.close()
      if args.force:
