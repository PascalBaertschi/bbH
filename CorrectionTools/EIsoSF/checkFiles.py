...
def main(args):
  ...
  for year in years:
    indir      = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/EIsoSF/%s%s%s/"%(ULtag,year,preVFP)
    samplesdir = args.outdir if args.outdir else "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/EIsoSF/samples_%s%s%s"%(ULtag,year,preVFP)
    os.chdir(indir)
    ...
    for channel in channels:
      ...
      if not args.haddother or args.make:
        for directory in samplelist:
            ...
            outdir  = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/EIsoSF/samples_%s%s%s"%(ULtag,year,preVFP)
      ...
      if args.haddother:
        if year==2016:
            ...
            workdir = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/EIsoSF"
            ...
            if JECvar=="": #for JECvariations no need to copy data
                ...
                for sample in ['SingleElectron_Run2016BCDEFGH']:
