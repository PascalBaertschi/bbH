#uthor: Izaak Neutelings (November, 2019)
...
#objectIds   = { t: i for i,t in objectTypes.iteritems() } python2.7
objectIds   = { t: i for i,t in objectTypes.items() } #python3.9
...
class TrigObjMatcher:
    ...
    def findL1Tau(self,event,ptcut,lepton,leptonid):
        lepton_trigObj = None
        deltaR = 0.2
        trigObjs_lepton = [o for o in Collection(event,'TrigObj') if o.id==leptonid]
        for trigObj_lep in trigObjs_lepton:
            if trigObj_lep.DeltaR(lepton)<deltaR: 
                deltaR = trigObj_lep.DeltaR(lepton)
                lepton_trigObj = trigObj_lep
        trigObjs = [o for o in Collection(event,'TrigObj') if o.id==15]
        for trigObj in trigObjs:
            if lepton_trigObj!=None:
                if trigObj.DeltaR(lepton_trigObj)>0.5 and trigObj.l1pt>ptcut:
                    return True
        return False

    def findL1Tau_reco(self,event,ptcut,lepton,leptonid):
        lepton_trigObj = None
        deltaR = 0.2
        trigObjs_lepton = [o for o in Collection(event,'TrigObj') if o.id==leptonid]
        for trigObj_lep in trigObjs_lepton:
        ...
        for trigObj in trigObjs:
            if lepton_trigObj!=None:
                ...
                if trigObj.DeltaR(lepton)>0.5 and trigObj.l1pt>ptcut:
                    return True
        return False
    ...
    def match_crosstrigger(self,event,recoObj,leg=1,dR=0.2):
        """Match given reconstructed object to trigger objects."""
        leg     -= 1 # index starting at 0
        trigObjs = [o for o in Collection(event,'TrigObj') if o.id==self.ids[leg]]
        for trigger in self.triggers:
          for trigObj in trigObjs:
            if trigger.filters[leg].matchbits(trigObj) and trigger.filters[leg].match(trigObj,recoObj,dR=dR):
              return True
