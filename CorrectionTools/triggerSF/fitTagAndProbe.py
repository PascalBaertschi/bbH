...
parser.add_argument('-tt', '--triggertag', dest='triggertag', action='store',type=str, default="")
...
parser.add_argument('--title', default='Single Muon Trigger Efficiency')
...
if variation in ["up","down","gen","tag","ele32ac","ele32deac"]:
    filename = "./root/%striggerEff_UL%s%s_%s_%s.root"%(trigger,year,preVFPtag,sample,variation)
else:
   filename = "./root/%striggerEff_UL%s%s_%s.root"%(trigger,year,preVFPtag,sample) 
   #filename = "./root/check/%striggerEff_UL%s%s_%s.root"%(trigger,year,preVFPtag,sample)
...
if variation!="":
    vartag = "_"+variation
filename_out = "./root/%strigger%sEff_UL%s%s_%s%s.root"%(trigger,triggertag,year,preVFPtag,sample,vartag)
#filename_out = "./root/check/%strigger%sEff_UL%s%s_%s%s.root"%(trigger,triggertag,year,preVFPtag,sample,vartag)
...
if "2018" in filename:
    ...
    if preVFPtag=="_preVFP":
        ...
        lumi = 16.8
if "etrigger" in filename:
    title = "Single Electron Trigger Efficiency"
elif "mutau" in filename:
    title = "Muon Cross Trigger Efficiency"
elif "etau" in filename:
    title = "Electron Cross Trigger Efficiency"

infile = ROOT.TFile(filename)
wsp = infile.Get('wsp%s_'%triggertag+name)
...
for b in bins:
    ...
    plot.DrawTitle(pads[0], 'Private work (CMS data)', 1)
    ...
    latex.DrawLatex(0.2, 0.89, 'Pass region')
    ...
    latex.SetTextSize(0.04)
    latex.DrawLatex(0.2, 0.89, 'Fail region')
    #plot.DrawTitle(pads[1], 'Fail Region', 1)
    #plot.DrawCMSLogo(pads[0], "Private work", "(CMS data)", 10, 0.045, 0.01, 1.0, '', 0.8)
    #plot.DrawCMSLogo(pads[0], "Private work", "(CMS data)", 10, 0.045, 0.01, 1.0, '', 0.8)
    ensureDirectory('%s/plots/%strigger_%s_UL%s%s%s/' % (args.plot_dir,trigger,sample,year,preVFPtag,triggertag))
    canv.Print('%s/plots/%strigger_%s_UL%s%s%s/%s%s.png' % (args.plot_dir,trigger,sample,year,preVFPtag,triggertag,canv.GetName(),vartag))
    canv.Print('%s/plots/%strigger_%s_UL%s%s%s/%s%s.pdf' % (args.plot_dir,trigger,sample,year,preVFPtag,triggertag,canv.GetName(),vartag))
