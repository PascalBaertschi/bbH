...
parser.add_argument('-y', '--year',    dest='years', choices=[2016,2017,2018], type=int, nargs='+', default=[2018], action='store',
                                       help="select year" )
parser.add_argument('-c', '--channel', dest='channels', choices=['mutau','etau','tautaug'], type=str, nargs='+', default=['mutau'], action='store',
...
def main():
    ...
    for year in years:
      ...
      samplelist = [ ]
      if JECvar!="":
          outdir     = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/triggerSF/%s%s%s_%s/"%(ULtag,year,preVFP,JECvar)
      else:
          outdir     = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/triggerSF/%s%s%s/"%(ULtag,year,preVFP)
      ...
      for channel in channels:
        print(header(year,preVFP,JECvar,channel,tag))
        
        for directory in samplelist:
            if JECvar!="":
                outdir       = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/triggerSF/%s%s%s_%s/%s"%(ULtag,year,preVFP,JECvar,directory)
            else:
                outdir       = "/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/triggerSF/%s%s%s/%s"%(ULtag,year,preVFP,directory)
