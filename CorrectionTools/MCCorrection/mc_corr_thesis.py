...
def drawPlots(hists,year,channel,preVFP):
    ...
    for sample in getsamples(channel,year,preVFP):
        ...
        if channel=="mumu":
            ...
            selection_1btag = 'isMuMu && nBjets_m==1'
            selection_2btag = 'isMuMu && nBjets_m>=2'
            selection_nobcut = 'isMuMu'
        elif channel=="tt":
            selection = 'isTTCR && nBjets_m>0 && Dzeta < -35.'
            selection_nobcut = 'isTTCR && Dzeta < -35.'
        elif channel=="mutau":
            selection = "isHtoMuTau && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Mu1_charge*Tau1_charge==-1 && mt > 80 && nBjets_m==0" #for correction of W+Jets sample
            if "WJets" in sample:
                selection_nobcut = "isHtoMuTau && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && mt>80" #use MCcorr and ff derivation selection for W+Jets sample
            else:
                selection_nobcut = "isHtoMuTau && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Tau1_genmatch!=6 && mt<60"
        elif channel=="etau":
            selection = "isHtoETau && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Ele1_charge*Tau1_charge==-1 && mt > 80 && nBjets_m==0" #for correction of W+Jets sample
            if "WJets" in sample:
                selection_nobcut = "isHtoETau && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && mt>80" #use MCcorr and ff derivation selection for W+Jets sample
            else:
                selection_nobcut = "isHtoETau && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Tau1_genmatch!=6 && mt<60"
            ...
            if channel=="mumu":
                if "TT" in sample:
                    ...
                    if "TTTo2L2Nu" in sample:
                       weight_up = 'EventWeight * LumiWeight *(93.5874/88.29) * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * wt_dy_nlo * wt_w_nlo *%s * wt_tt_pt'%tt_weight
                       weight_down =  'EventWeight * LumiWeight * (82.9926/88.29) * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * wt_dy_nlo * wt_w_nlo *%s * wt_tt_pt'%tt_weight 
                    elif "TTToSemiLeptonic" in sample:
                       weight_up = 'EventWeight * LumiWeight * (397.2604/365.34) * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * wt_dy_nlo * wt_w_nlo *%s * wt_tt_pt'%tt_weight
                       weight_down =  'EventWeight * LumiWeight * (343.3196/365.34) * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * wt_dy_nlo * wt_w_nlo *%s * wt_tt_pt'%tt_weight
                    elif "TTToHadronic" in sample:
                       weight_up = 'EventWeight * LumiWeight * (400.6376/377.96) * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * wt_dy_nlo * wt_w_nlo *%s * wt_tt_pt'%tt_weight
                       weight_down =  'EventWeight * LumiWeight * (355.2824/377.96) * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * wt_dy_nlo * wt_w_nlo *%s * wt_tt_pt'%tt_weight
                    #####
                else:
                ...
                if "TT" in sample:
                    weight = 'EventWeight * LumiWeight * BTagWeight * PUIdWeight * MuWeight * MuTriggerWeight * EWeight * wt_dy_nlo * wt_w_nlo * wt_tt_pt' #MuTriggerWeight includes SingleElectron trigger   
        ...
        if channel=="mumu":
                ...
                #####
                hists["2D"][sample+"_up"]= Hist('TH2F', sample=sample, var=["vis_pt","vis_mass"], binning=(20,0.,200.,16,50.,130.), sel=selection, wt=weight_up)
                hists["2D"][sample+"_down"]= Hist('TH2F', sample=sample, var=["vis_pt","vis_mass"], binning=(20,0.,200.,16,50.,130.), sel=selection, wt=weight_down)
                hists["vis_pt"][sample+"_up"]= Hist('TH1F', sample=sample, var=["vis_pt"], binning=(20,0.,200.), sel=selection, wt=weight_up)
                hists["vis_pt"][sample+"_down"]= Hist('TH1F', sample=sample, var=["vis_pt"], binning=(20,0.,200.), sel=selection, wt=weight_down)
                hists["vis_mass"][sample+"_up"]= Hist('TH1F', sample=sample, var=["vis_mass"], binning=(16,50.,130.), sel=selection, wt=weight_up)
                hists["vis_mass"][sample+"_down"]= Hist('TH1F', sample=sample, var=["vis_mass"], binning=(16,50.,130.), sel=selection, wt=weight_down)
                hists["2D_1btag"][sample+"_up"]= Hist('TH2F', sample=sample, var=["vis_pt","vis_mass"], binning=(11,np.array(bins_pt),8,np.array(bins_mass)), sel=selection_1btag, wt=weight_up)
                hists["2D_2btag"][sample+"_up"]= Hist('TH2F', sample=sample, var=["vis_pt","vis_mass"], binning=(11,np.array(bins_pt),8,np.array(bins_mass)), sel=selection_2btag, wt=weight_up)
                hists["2D_1btag"][sample+"_down"]= Hist('TH2F', sample=sample, var=["vis_pt","vis_mass"], binning=(11,np.array(bins_pt),8,np.array(bins_mass)), sel=selection_1btag, wt=weight_down)
                hists["2D_2btag"][sample+"_down"]= Hist('TH2F', sample=sample, var=["vis_pt","vis_mass"], binning=(11,np.array(bins_pt),8,np.array(bins_mass)), sel=selection_2btag, wt=weight_down)
                hists["vis_pt_1btag"][sample+"_up"]= Hist('TH1F', sample=sample, var=["vis_pt"], binning=bins_pt, sel=selection_1btag, wt=weight_up)
                hists["vis_pt_2btag"][sample+"_up"]= Hist('TH1F', sample=sample, var=["vis_pt"], binning=bins_pt, sel=selection_2btag, wt=weight_up)
                hists["vis_mass_1btag"][sample+"_up"]= Hist('TH1F', sample=sample, var=["vis_mass"], binning=bins_mass, sel=selection_1btag, wt=weight_up)
                hists["vis_mass_2btag"][sample+"_up"]= Hist('TH1F', sample=sample, var=["vis_mass"], binning=bins_mass, sel=selection_2btag, wt=weight_up)
                hists["vis_pt_1btag"][sample+"_down"]= Hist('TH1F', sample=sample, var=["vis_pt"], binning=bins_pt, sel=selection_1btag, wt=weight_down)
                hists["vis_pt_2btag"][sample+"_down"]= Hist('TH1F', sample=sample, var=["vis_pt"], binning=bins_pt, sel=selection_2btag, wt=weight_down)
                hists["vis_mass_1btag"][sample+"_down"]= Hist('TH1F', sample=sample, var=["vis_mass"], binning=bins_mass, sel=selection_1btag, wt=weight_down)
                hists["vis_mass_2btag"][sample+"_down"]= Hist('TH1F', sample=sample, var=["vis_mass"], binning=bins_mass, sel=selection_2btag, wt=weight_down)
                #####################
                ...
                hists["vis_pt"][sample+'_1btag']= Hist('TH1F', sample=sample, var=["vis_pt"], binning=(20,0.,200.), sel=selection_1btag, wt=weight)
                hists["vis_mass"][sample+'_1btag']= Hist('TH1F', sample=sample, var=["vis_mass"], binning=(16,50.,130.), sel=selection_1btag, wt=weight)
                hists["vis_pt"][sample+'_2btag']= Hist('TH1F', sample=sample, var=["vis_pt"], binning=(20,0.,200.), sel=selection_2btag, wt=weight)
                hists["vis_mass"][sample+'_2btag']= Hist('TH1F', sample=sample, var=["vis_mass"], binning=(16,50.,130.), sel=selection_2btag, wt=weight)
                if "DYJets" in sample:
        ...
        hists["nJets"][sample]= Hist('TH1F', sample=sample, var=["nJets"], binning=(10,0,10), sel=selection, wt=weight)
        ...
        hists["nJets"][sample + '_btagSF']= Hist('TH1F', sample=sample, var=["nJets"], binning=(10,0,10), sel=selection_nobcut, wt=weight_btagSF)
        ...
        hists["nJets"][sample + '_nobtagSF']= Hist('TH1F', sample=sample, var=["nJets"], binning=(10,0,10), sel=selection_nobcut, wt=weight_nobtagSF)
...
if channel=="mumu":
    ...
    ####
    hists_2D["data_obs_up"].Add(hists_2D["TTTo2L2Nu_up"],-1)
    hists_2D["data_obs_up"].Add(hists_2D["TTToSemiLeptonic_up"],-1)
    hists_2D["data_obs_up"].Add(hists_2D["TTToHadronic_up"],-1)
    hists_2D["data_obs_down"].Add(hists_2D["TTTo2L2Nu_down"],-1)
    hists_2D["data_obs_down"].Add(hists_2D["TTToSemiLeptonic_down"],-1)
    hists_2D["data_obs_down"].Add(hists_2D["TTToHadronic_down"],-1)
    #####
    ...
    #####
    hists_2D_1btag["data_obs_up"].Add(hists_2D_1btag["TTTo2L2Nu_up"],-1)
    hists_2D_1btag["data_obs_up"].Add(hists_2D_1btag["TTToSemiLeptonic_up"],-1)
    hists_2D_1btag["data_obs_up"].Add(hists_2D_1btag["TTToHadronic_up"],-1)
    hists_2D_1btag["data_obs_down"].Add(hists_2D_1btag["TTTo2L2Nu_down"],-1)
    hists_2D_1btag["data_obs_down"].Add(hists_2D_1btag["TTToSemiLeptonic_down"],-1)
    hists_2D_1btag["data_obs_down"].Add(hists_2D_1btag["TTToHadronic_down"],-1)
    #####
    ...
    #####
    hists_2D_2btag["data_obs_up"].Add(hists_2D_2btag["TTTo2L2Nu_up"],-1)
    hists_2D_2btag["data_obs_up"].Add(hists_2D_2btag["TTToSemiLeptonic_up"],-1)
    hists_2D_2btag["data_obs_up"].Add(hists_2D_2btag["TTToHadronic_up"],-1)
    hists_2D_2btag["data_obs_down"].Add(hists_2D_2btag["TTTo2L2Nu_down"],-1)
    hists_2D_2btag["data_obs_down"].Add(hists_2D_2btag["TTToSemiLeptonic_down"],-1)
    hists_2D_2btag["data_obs_down"].Add(hists_2D_2btag["TTToHadronic_down"],-1)
    ####
...
for path, node in hists.ListNodes(withObjects=True):
    ...
    if channel=="mumu":
        ...
        ####
        if path in ["vis_pt","vis_mass","vis_pt_1btag","vis_pt_2btag","vis_mass_1btag","vis_mass_2btag"]:
                node['TT_up'] = node['TTTo2L2Nu_up'] + node['TTToHadronic_up'] + node['TTToSemiLeptonic_up']
                node['TT_down'] = node['TTTo2L2Nu_down'] + node['TTToHadronic_down'] + node['TTToSemiLeptonic_down']
                node['dataMinusMC_up'] = node['data_obs']-node['TT_up']-node['ST'] 
                node['dataMinusMC_down'] = node['data_obs']-node['TT_down']-node['ST'] 
        #####
    elif channel=="mutau" or channel=="etau":
