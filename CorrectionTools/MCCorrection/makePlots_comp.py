...
def MakeStackedPlot(name, outdir, hists,selection, cfg):
    if selection=="mumu":
      ...
      #bkgscheme =  [processComp("TT no btag SF",["TTTo2L2Nu_nobtagSF"],plot.CreateTransparentColor(TColor.GetColor(187,5,30), 0.5)),processComp("TT btag SF",["TTTo2L2Nu_btagSF"],plot.CreateTransparentColor(TColor.GetColor(0,153,76), 0.5))]
    copyhists = {}
    ...
    pads = plot.TwoPadSplit(0.13,0.01,0.01)
    h_data = hists['DYJets']
    ...
    if "1btag" in name:
      latex.DrawLatex(0.20, 0.75, channel_text+", 1b jet")
    elif "2btag" in name:
      latex.DrawLatex(0.20, 0.75, channel_text+", 2b jets")
    else:
