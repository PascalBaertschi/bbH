...
class Producer(Module):
    ...
    def analyze(self, event):
        ...
        for tau in taus:
            ...
            if abs(tau.charge) == 1 and abs(tau.dz) < 0.2 and tau_idDecayMode and tau.idDeepTau2017v2p1VSjet >= 31:
        ...
        # only keep event if transverse mass smaller than 60 GeV
        ...
        # only keep event if at least one medium b-tagged jet
        ...
        for i,jet in enumerate(jets):
            ...
            if abs(jet.eta) < 2.4 and jet.pt > 20. and jet.jetId >= 2 and (jet.puId != 0 or jet.pt > 50.) and jet.btagDeepFlavB > wp_medium:
                if self.vec(jet).DeltaR(self.vec(pair[0])) > 0.4: #don't check deltaR to tau to avoid bias
