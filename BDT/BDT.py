...
parser.add_argument('--pearson', dest="pearson", action='store_true', default=False)
...
if args.evaluate:
    ...
    """
    if year=="2018":
        y_onehot_test = y_onehot_test_2018
        y_score = y_score_2018
    elif year=="2017":
        y_onehot_test = y_onehot_test_2017
        y_score = y_score_2017
    elif year=="2016":
        y_onehot_test = y_onehot_test_2016
        y_score = y_score_2016
    
    fig_roc, ax_roc = pyplot.subplots(figsize=(8,8))
    RocCurveDisplay.from_predictions(
        y_onehot_test[:, 0],
        y_score[:, 0],
    name=f"Signal vs the rest",
    color="darkorange",
    ax=ax_roc,
    )
    RocCurveDisplay.from_predictions(
    ...
    pyplot.xlabel("False Positive Rate")
    pyplot.ylabel("True Positive Rate")
    pyplot.title("ROC curves %s %s"%(year,channel))
    pyplot.legend()
    pyplot.savefig('plots/roc_%s_UL%s.png'%(channel,year))
...
if args.pearson:
    ...
    file_path = "pearson_coeff_signal_%s_UL%s.txt"%(channel,year)
    file_sig = open(file_path,"w")
    for i in range(14):
        for j in range(14):
            if i==j: continue
            var_1 = inputlist[i]
            var_2 = inputlist[j]
            text_line = "%s   %s - %s: \n"%(round(pearsonr(signal_train[var_1],signal_train[var_2])[0],4),var_1,var_2)
            file_sig.write(text_line)
    file_sig.close()

    file_path = "pearson_coeff_TT_%s_UL%s.txt"%(channel,year)
    file_tt = open(file_path,"w")
    for i in range(14):
        for j in range(14):
            if i==j: continue
            var_1 = inputlist[i]
            var_2 = inputlist[j]
            text_line = "%s   %s - %s: \n"%(round(pearsonr(tt_train[var_1],tt_train[var_2])[0],4),var_1,var_2)
            file_tt.write(text_line)
    file_tt.close()
    
    file_path = "pearson_coeff_DY_%s_UL%s.txt"%(channel,year)
    file_dy = open(file_path,"w")
    for i in range(14):
        for j in range(14):
            if i==j: continue
            var_1 = inputlist[i]
            var_2 = inputlist[j]
            text_line = "%s   %s - %s: \n"%(round(pearsonr(dy_train[var_1],dy_train[var_2])[0],4),var_1,var_2)
            file_dy.write(text_line)
    file_dy.close()
