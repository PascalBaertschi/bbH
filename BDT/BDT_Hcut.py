...
outdir = ensureDirectory("/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples_UL%s%s%s_BDT/%s_Hcut/"%(year,preVFP,sysvar,channel))
...
modelname = 'xgbmodel_%s_UL%s_Hcut.sav'%(channel,year)
...
if channel=="mutau":
    selection_test = '(isHtoMuTau || isHtoMuTauAR) && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && EventNumber%10>=4 && (H_mass > 100. && H_mass < 140.)' #use Events with endnumber 4 5 6 7 8 9 for testing BDT
    selection_test_data = '(isHtoMuTau || isHtoMuTauAR) && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && transverse_mass_lepmet<60 && nBjets_m>=1 && (H_mass > 100. && H_mass < 140.)' #use Events with endnumber 4 5 6 7 8 9 for testing BDT
elif channel=="etau":
    selection_test = '(isHtoETau || isHtoETauAR) && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && EventNumber%10>=4 && (H_mass > 100. && H_mass < 140.)' #use Events with endnumber 4 5 6 7 8 9 for testing BDT
    selection_test_data = '(isHtoETau || isHtoETauAR) && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && (H_mass > 100. && H_mass < 140.)' #use Events with endnumber 4 5 6 7 8 9 for testing BDT


if args.train:
    file_signal_train = uproot.open("%sBDT_UL%s_%s_train_signal_Hcut.root"%(rootdir,year,channel))
    ...
    file_tt_train = uproot.open("%sBDT_UL%s_%s_train_tt_Hcut.root"%(rootdir,year,channel))
    file_dy_train = uproot.open("%sBDT_UL%s_%s_train_dy_Hcut.root"%(rootdir,year,channel))
    ...
    file_signal_test = uproot.open("%sBDT_UL%s_%s_test_signal_Hcut.root"%(rootdir,year,channel))
    ...
    file_tt_test = uproot.open("%sBDT_UL%s_%s_test_tt_Hcut.root"%(rootdir,year,channel))
    file_dy_test = uproot.open("%sBDT_UL%s_%s_test_dy_Hcut.root"%(rootdir,year,channel))
    ...
    pyplot.savefig('plots/feature_importance_weight_%s_Hcut'%name,dpi=300)
    
    xgb.plot_importance(model,importance_type="cover",show_values=False)
    pyplot.savefig('plots/feature_importance_cover_%s_Hcut'%name,dpi=300)

    xgb.plot_importance(model,importance_type="gain",show_values=False)
    pyplot.savefig('plots/feature_importance_gain_%s_Hcut'%name,dpi=300)
    ...
    pyplot.savefig('plots/feature_importance_shap_%s_Hcut'%name,dpi=300)
