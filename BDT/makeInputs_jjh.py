...
#mc_samples = ['TTTo2L2Nu','TTToHadronic','TTToSemiLeptonic','bbH_htt','ggH_bb_htt_inc','ggH_bb_htt_excl','DYJets_incl','DYJets_0J','DYJets_1J','DYJets_2J','bbH_nobb_htt','ggH_htt_excl','ggH_htt_inc','VBF','ZH','ttH'] 
mc_samples = ['bbH_nobb_htt','ggH_htt_excl','ggH_htt_inc','VBF','ZH','ttH']
#mc_samples = ['bbH_nobb_htt','ggH_htt_excl','ggH_htt_inc'] 
...
if channel=="mutau":
    selection_train = 'isHtoMuTau && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && EventNumber%10<=3' #use Events with endnumber 0 1 2 3 for training BDT
    selection_test = 'isHtoMuTau && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && EventNumber%10>=4'  #use Events with endnumber 4 5 6 7 8 9 for testing BDT
elif channel=="etau":
    selection_train = 'isHtoETau && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && EventNumber%10<=3' #use Events with endnumber 0 1 2 3 for training BDT
    selection_test = 'isHtoETau && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Tau1_genmatch!=6 && transverse_mass_lepmet<60 && nBjets_m>=1 && EventNumber%10>=4' #use Events with endnumber 4 5 6 7 8 9 for testing BDT
...
def BDTinput(sample_shortname,filetype,year,preVFP):
    ...
    isjjHBranch = skimmed_tree.Branch('isjjH', isjjH, 'isjjH/I')
    ...
    for event in range(0,skimmed_tree.GetEntries()):
        ...
        if sample_shortname in ['bbH_htt','ggH_bb_htt_inc','ggH_bb_htt_excl','intH_bb_htt']:
            ...
            isDY[0] = 1
            BDTtarget[0] = 2
        elif sample_shortname in ['bbH_nobb_htt','ggH_htt_excl','ggH_htt_inc','VBF','ZH','ttH']:
            isjjH[0] = 1
            BDTtarget[0] = 3
        else:
            BDTtarget[0] = 4
    ...
    if filetype=="train":
        if "TTTo" in sample:
            ...
            jjh_chain_train.Add("%s%s_skimmed_train_UL%s%s.root"%(outdir,sample,year,preVFP))
    else:
        if "TTTo" in sample:
            ...
            jjh_chain_test.Add("%s%s_skimmed_test_UL%s%s.root"%(outdir,sample,year,preVFP))
...
if __name__=="__main__":
    ...
    if NN:
        ...
        jjh_chain_train.Merge("%sBDT_UL%s_%s_train_jjh.root"%(outdir,year,channel))
        ...
        jjh_chain_test.Merge("%sBDT_UL%s_%s_test_jjh.root"%(outdir,year,channel))
