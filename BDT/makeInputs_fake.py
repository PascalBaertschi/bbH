...
mc_samples = ['data_obs','TTTo2L2Nu','TTToHadronic','TTToSemiLeptonic'] #intH_bb_htt
...
if channel=="mutau":
    ...
    selection_train_ar = 'isHtoMuTauAR && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Mu1_charge*Tau1_charge==-1 && Tau1_genmatch!=6 && nBjets_m>=1 && mt<60 && EventNumber%10<=3' #use Events with endnumber 0 1 2 3 for training BDT
    selection_test_ar = 'isHtoMuTauAR && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Mu1_charge*Tau1_charge==-1 && Tau1_genmatch!=6 && nBjets_m>=1 && mt<60 && EventNumber%10>=4'  #use Events with endnumber 4 5 6 7 8 9 for testing BDT
    ...
    selection_train_ar = 'isHtoETauAR && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Ele1_charge*Tau1_charge==-1 && nBjets_m>=1 && mt<60 && EventNumber%10<=3'
    selection_test_ar = 'isHtoETauAR && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Ele1_charge*Tau1_charge==-1 && nBjets_m>=1 && mt<60 && EventNumber%10>=4'
...
def BDTinput(sample_shortname,filetype,year,preVFP):
    ...
    if filetype=="train":
        if sample_shortname=="data_obs":
            print("take AR selection train")
            selection = selection_train_ar
        else:
            selection = selection_train
    else:
        if sample_shortname=="data_obs":
            print("take AR selection test")
            selection = selection_test_ar
        else:
    ...
    #isFakeBranch = skimmed_tree.Branch('isFake', isFake, 'isFake/I')
    ...
    for event in range(0,skimmed_tree.GetEntries()):
        ...
        if sample_shortname in ['bbH_htt','ggH_bb_htt_inc','ggH_bb_htt_excl','intH_bb_htt']:
            isSignal[0] = 1
            BDTtarget[0] = 0
        elif "TTTo" in sample or sample_shortname=="data_obs":
            isTT[0] = 1
            BDTtarget[0] = 1
        elif sample_shortname in ['DYJets_incl','DYJets_0J','DYJets_1J','DYJets_2J']:
            isDY[0] = 1
            BDTtarget[0] = 2
        #elif sample_shortname=="data_obs":
        #    isFake[0] = 1
        #    BDTtarget[0] = 3
        else:
            BDTtarget[0] = 3
    ...
    outfile.Close()
    if filetype=="train":
        if "TTTo" in sample or sample_shortname=="data_obs":
        ...
        #elif sample_shortname=="data_obs":
        #    fake_chain_train.Add("%s%s_skimmed_train_UL%s%s.root"%(outdir,sample,year,preVFP))
    else:
        if "TTTo" in sample or sample_shortname=="data_obs":
        ...
        #elif sample_shortname=="data_obs":
        #    fake_chain_test.Add("%s%s_skimmed_test_UL%s%s.root"%(outdir,sample,year,preVFP))
...
if __name__=="__main__":
    ...
    if NN:
        ...
        tt_chain_train.Merge("%sBDT_UL%s_%s_train_tt_fake.root"%(outdir,year,channel))
        #dy_chain_train.Merge("%sBDT_UL%s_%s_train_dy.root"%(outdir,year,channel))
        #fake_chain_train.Merge("%sBDT_UL%s_%s_train_fake.root"%(outdir,year,channel))
        #signal_chain_test.Merge("%sBDT_UL%s_%s_test_signal.root"%(outdir,year,channel))
        tt_chain_test.Merge("%sBDT_UL%s_%s_test_tt_fake.root"%(outdir,year,channel))
        #dy_chain_test.Merge("%sBDT_UL%s_%s_test_dy.root"%(outdir,year,channel))
        #fake_chain_test.Merge("%sBDT_UL%s_%s_test_fake.root"%(outdir,year,channel))
