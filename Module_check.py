...
class Producer(Module):
    ...
    def analyze(self, event):
        ...
        for i,jet in enumerate(jets):
            ...
            if self.isHtoETau:
                jet_pt_flavorqcd_up = eval("event.Jet_pt_jesFlavorQCDUp[i]")
                jet_pt_flavorqcd_down = eval("event.Jet_pt_jesFlavorQCDDown[i]")
                jet_pt_absolute_up = eval("event.Jet_pt_jesAbsoluteUp[i]")
                jet_pt_absolute_down = eval("event.Jet_pt_jesAbsoluteDown[i]")
                flavorqcd_up_diff = jet.pt - jet_pt_flavorqcd_up
                flavorqcd_down_diff = jet.pt - jet_pt_flavorqcd_down
                absolute_up_diff = jet.pt - jet_pt_absolute_up
                absolute_down_diff = jet.pt - jet_pt_absolute_down
                flavor_qcd_onesided = False
                absolute_onesided = False
                if (flavorqcd_up_diff > 0. and flavorqcd_down_diff > 0.) or (flavorqcd_up_diff < 0. and flavorqcd_down_diff < 0.):
                    flavor_qcd_onesided = True
                if (absolute_up_diff > 0. and absolute_down_diff > 0.) or (absolute_up_diff < 0. and absolute_down_diff < 0.):
                    absolute_onesided = True
                print("flavor qcd onesided:",flavor_qcd_onesided," absolute onesided:",absolute_onesided)
            if "jes" in self.JECvar:
