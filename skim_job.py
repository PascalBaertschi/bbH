...
parser.add_argument('-T', '--tes',     dest='tes', action='store', type=str, default="no",choices=["yes","no"])
...
tree_number = infiles[0].split("/")[-1].split("_")[1].split(".")[0]
fname = "%s/tree_%i.root"%(outdir,args.nchunck)
outdir_scratch = '/scratch/pbaertsc/bbh/%s_skim/%s_%i'%(year,outfile,args.nchunck)
fname_scratch = "%s/tree_%s_Skim.root"%(outdir_scratch,tree_number)
ensureDirectory(outdir_scratch)

print('-'*80)
print("%-12s = %s"%('input files',infiles))
print("%-12s = %s"%('treeNumber',tree_number))
...
if tes=="yes":
  ...
  print("running skimming for TES...")
elif JECvar=="yes":
  from skim_Module import *
  print("running skimming for JEC...")
