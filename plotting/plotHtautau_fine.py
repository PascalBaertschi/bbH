...
if isBDT:
    drawvars = [    
        ...
        ('BDTisTT', 'BDTisTT',          [0,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.]),
        ('BDTisDY', 'BDTisDY',          [0,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.]),
        ...
        if sysvar=="":
            ...
            if isBDT:
                ...
                if year in ["2017","2016"]:
                    hists[dirname][sample + '_AR_prefiringUp'] = Hist('TH1F', sample=sample, var=[var], binning=binning, sel=selection_ar, wt=weight_ar+' * PrefireWeight_up')
                    hists[dirname][sample + '_AR_prefiringDown'] = Hist('TH1F', sample=sample, var=[var], binning=binning, sel=selection_ar, wt=weight_ar+' * PrefireWeight_down')
...
if isBDT and sysvar=="":
    ...
    if year in ["2017","2016"]:
        variations_list = variations_list + ["prefiringUp","prefiringDown"]
elif sysvar!="":
    variations_list = [""]
else:
...
for path, node in hists.ListNodes(withObjects=True):
    ...
    if isBDT:
        ...
        if year in ["2017","2016"]:
            variations_list_AR = variations_list_AR + ["prefiringUp","prefiringDown"]
    else:
...
if isBDT:
    filename = './root/%s_UL%s%s%s_finebinning_BDT.root'%(channel,year,preVFP,sysvar_noyear)
    #filename = './root/%s_UL%s%s%s_BDT_feb2023.root'%(channel,year,preVFP,sysvar_noyear)
else:
