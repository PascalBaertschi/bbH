...
def MakeStackedPlot(name, outdir, hists, cfg,sys):
    ...
    #bkgscheme =  [processComp("TT",["TT"],plot.CreateTransparentColor(TColor.GetColor(255,0,255),0.3)),processComp("TT_jesFlavorQCDUp",["TT_jesFlavorQCDUp"],plot.CreateTransparentColor(TColor.GetColor(155, 152, 204),0.3)),processComp("TT_jesFlavorQCDDown",["TT_jesFlavorQCDDown"],plot.CreateTransparentColor(TColor.GetColor(187, 5, 30),0.3))]
    bkgscheme =  [processComp("TT",["TT"],plot.CreateTransparentColor(TColor.GetColor(255,0,255),0.3)),processComp("TT_%sUp"%sys,["TT_%sUp"%sys],plot.CreateTransparentColor(TColor.GetColor(155, 152, 204),0.3)),processComp("TT_%sDown"%sys,["TT_%sDown"%sys],plot.CreateTransparentColor(TColor.GetColor(187, 5, 30),0.3))]
    ...
    if name=="collinear_mass":
          print("hist norm for: data from ",h_data.GetXaxis().GetBinCenter(1),"to ",h_data.GetXaxis().GetBinCenter(h_data.GetNbinsX()))
          print("is ",h_data.Integral())
    ...
    h_nom = hists["TT"]
    h_up = hists["TT_%sUp"%sys]
    h_down = hists["TT_%sDown"%sys]
    #h_up = hists["TT_jesBBEC1Up"]
    #h_down = hists["TT_jesBBEC1Down"]    
    #### end comment here ####
    """
    ### only bkg plot #####
    # Build overlays
    ...
    r_up = plot.MakeRatioHist(h_nom, h_up, True, False)
    r_up.SetMarkerColor(TColor.GetColor(155, 152, 204))
    r_up.SetFillColor(plot.CreateTransparentColor(17,0.3))
    #r_up.GetYaxis().SetRangeUser(0.9,1.1)
    r_up.GetYaxis().SetRangeUser(0.95,1.05)
    r_up.GetXaxis().SetTitle(x_title)
    r_up.Draw('E2SAME')
    r_down = plot.MakeRatioHist(h_nom, h_down, True, False)
    r_down.SetMarkerColor(TColor.GetColor(187, 5, 30))
    r_down.SetFillColor(plot.CreateTransparentColor(17,0.3))
    r_down.Draw('E2SAME')
    ...
    latex = ROOT.TLatex()
    norm_nominal = h_nom.Integral()
    norm_up = h_up.Integral()
    norm_down = h_down.Integral()
    plot.Set(latex, NDC=None, TextFont=42, TextSize=0.03)
    latex.DrawLatex(0.20, 0.75, channel_text)
    #latex.DrawLatex(0.20, 0.60, "norm up: %.1f"%norm_up)
    #latex.DrawLatex(0.20, 0.56, "diff %.1f"%(norm_up-norm_nominal))
    #latex.DrawLatex(0.20, 0.52, "norm nom: %.1f"%norm_nominal)
    #latex.DrawLatex(0.20, 0.48, "diff %.1f"%(norm_down-norm_nominal))            
    #latex.DrawLatex(0.20, 0.44, "norm down: %.1f"%norm_down)
    
    #latex.DrawLatex(0.20, 0.60, "mean up: %.4f"%h_up.GetMean())
    #latex.DrawLatex(0.20, 0.56, "mean nom: %.4f"%h_nom.GetMean())            
    #latex.DrawLatex(0.20, 0.52, "mean down: %.4f"%h_down.GetMean())
...
parser.add_argument('--output', '-o', default='./plots/BDT', help='top level output directory')
...
config_by_setting = {
  "x_title": [
    ...
    "hide_data": [('H_mass', True),('vis_mass', False),('BDToutput', False),('BDTisSignal', False),('BDTisTT', False),('BDTisDY', False),('BDTisfake', False),('BDToutSig',True),('BDToutTT',True),('BDToutDY',True),('BDTisBkg', False), ('BDToutBkg', False),('MET', False)],
    ...
    "logy_min":[('BDToutput', 1),('BDTisSignal', 1),('BDTisTT', 1),('BDTisDY', 1),('BDTisfake', 1),('BDToutSig', 1),('BDToutTT', 100),('BDToutDY', 100),('BDTCR', 1),('BDTisBkg', 1), ('BDToutBkg', 1)],
    }
    # "ratio_range" :[
    #     ('vis_mass', [0,5])]}
...
#for sys in ["jesFlavorQCD","jesAbsolute","met_unclustered2018","scale_t_1prong2018","ff2018_qcd","jesRelativeSample_2018"]:
#for sys in ["jesFlavorQCD","jesAbsolute","met_unclustered2017","scale_t_1prong2017","ff2017_qcd","jesRelativeSample_2017"]:
for sys in ["jesFlavorQCD","jesAbsolute","met_unclustered2016","scale_t_1prong2016","ff2016_qcd","jesRelativeSample_2016"]:
  for path, subnode in node.ListNodes(withObjects=True):
    if path not in ["BDToutSig","BDToutDY","BDToutTT","Jet1_pt"]: continue
    split_path = path.split('/')[:-1]
    name = path.split('/')[-1]
    target_dir = os.path.join(output, *split_path)+sys
    ...
    MakeStackedPlot(name, target_dir, hists, plotcfg,sys)
