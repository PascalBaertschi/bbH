...
def MakeStackedPlot(name, outdir, hists, cfg):
    ...
    #WJets TColor.GetColor("#e42536")
    #DYJets TColor.GetColor("#f89c20")
    #ST TColor.GetColor("#832db6")
    #TT TColor.GetColor("#f89c20")
    #VV TColor.GetColor("#b9ac70")
    #ZH TColor.GetColor("#a96b59")
    #VBF TColor.GetColor("#717581")
    #ttH TColor.GetColor("#92dadd")
    #old colors:
    ...
    #bkgscheme =  [processComp("jet#rightarrow#tau_{h} fakes",["QCD"],TColor.GetColor(0,153,76)),processComp("WJets",["WJets_incl","WJets_0J","WJets_1J","WJets_2J"],TColor.GetColor(222,90,106)),processComp("DYJets",["DYJets_incl","DYJets_0J","DYJets_1J","DYJets_2J"], TColor.GetColor(248, 206, 104)),processComp("ST",["ST"], TColor.GetColor(208,240,193)),processComp("TT",["TT"], TColor.GetColor(155, 152, 204)),processComp("VV",["VV"],TColor.GetColor(111,45,53)),processComp("VH",["ZH","WH"],TColor.GetColor(255, 163, 4)),processComp("VBF",["VBF"],TColor.GetColor(187, 5, 30)),processComp("ttH",["ttH"],TColor.GetColor(255, 0, 255))]
    #bkgscheme =  [processComp("WJets",["WJets_incl","WJets_0J","WJets_1J","WJets_2J"],TColor.GetColor(222,90,106))]
    bkgscheme = [
        processComp("H(125)",["ZH","WH","VBF","ttH"],TColor.GetColor("#5790fc")),
        processComp("Electroweak",["ST","VV"],TColor.GetColor("#e42536")),
        processComp("jet#rightarrow#tau_{h} misid",["QCD"],TColor.GetColor("#9c9ca1")),
     	processComp("t#bar{t}",["TT"], TColor.GetColor("#964a8b")),
     	processComp("Z+jets",["DYJets_incl","DYJets_0J","DYJets_1J","DYJets_2J"], TColor.GetColor("#f89c20"))]
    #old colors:
    #bkgscheme = [
    #    processComp("H(125)",["ZH","WH","VBF","ttH"],TColor.GetColor(250, 202, 255)),
    #    processComp("electroweak",["ST","VV"],TColor.GetColor(222,90,106)),
    #    processComp("jet#rightarrow#tau_{h} fakes",["QCD"],TColor.GetColor("#c6f74a")),
    # 	processComp("t#bar{t}",["TT"], TColor.GetColor(155, 152, 204)),
    # 	processComp("Drell-Yan",["DYJets_incl","DYJets_0J","DYJets_1J","DYJets_2J"], TColor.GetColor(248, 206, 104))]
    ...
    signal = [processCompSignal("bbH#tau#tau(y_{b}^{2})",["bbHtt"],4,1,50),processCompSignal("bbH#tau#tau(y_{t}^{2})",["jjHtt"],4,2,50)]
    ...
    if name == "BDToutSig":
      ...
      hist_bkg = h_tot
      hist_DYJets = hists["DYJets"]
      hist_TT = hists["TT"]
      hist_QCD = hists["QCD"]
      Nbins = hist_bbH.GetNbinsX()
      for i in range(Nbins):
        i = Nbins-i
        bin_bbH = round(hist_bbH.GetBinContent(i),4)
        error_sig = round(hist_bbH.GetBinError(i),4)
        bin_bkg = round(hist_bkg.GetBinContent(i),4)
        error_bkg = round(hist_bkg.GetBinError(i),4)
        bin_DYJets = round(hist_DYJets.GetBinContent(i),4)
        bin_TT = round(hist_TT.GetBinContent(i),4)
        bin_QCD = round(hist_QCD.GetBinContent(i),4)
        low_edge = round(hist_bbH.GetBinLowEdge(i),5)
        high_edge = round(hist_bbH.GetBinLowEdge(i+1),5)
        print(low_edge,"-",high_edge,"  signal:",bin_bbH," bkg:", bin_bkg," TT:",bin_TT," DYJets:",bin_DYJets," fake taus:",bin_QCD)
        #print("bin_bbH:",bin_bbH," error:",error_sig)
        #if bin_bbH!=0. and bin_bkg!=0.:
        #  #print(low_edge,"-",high_edge," sig unc:",round(error_sig/bin_bbH,4)," bkg unc:",round(error_bkg/bin_bkg,4))
        #  print(low_edge,"-",high_edge,"sig:",round(error_sig/bin_bbH,4),"       bkg:",round(error_bkg/bin_bkg,4))
    #######################
    ...
    plot.DrawCMSLogo(pads[0], "Private work", "(CMS data/simulation)", 10, 0.045, 0.05, 1.0, '', 0.8) #Comment for PR plot
    ...
    if name in ["H_mass","collinear_mass","Jet1_btag","Jet1_pt","TauJ_mass","mt"]:
      canv.Print(split_outdir[0]+"/"+split_outdir[1]+'/thesis/'+ name + '_%s.pdf' %split_outdir[2][:-4])
...
config_by_setting = {
  "x_title": [
    ...
    "hide_data": [('H_mass', False),('vis_mass', False),('BDToutput', False),('BDTisSignal', False),('BDTisTT', False),('BDTisDY', False),('BDTisfake', False),('BDToutSig',False),('BDToutTT',False),('BDToutDY',False),('BDTisBkg', False), ('BDToutBkg', False),('MET', False)],
    ...
    "logy_min":[('BDToutput', 1),('BDTisSignal', 1),('BDTisTT', 1),('BDTisDY', 1),('BDTisfake', 1),('BDToutSig', 1),('BDToutTT', 1),('BDToutDY', 1),('BDTCR', 1),('BDTisBkg', 1), ('BDToutBkg', 1)],
    }
    # "ratio_range" :[
    #     ('vis_mass', [0,5])]}
...
for path, subnode in node.ListNodes(withObjects=True):
  ...
  if "2D" in path:
    ...
    continue
  if path not in ["H_mass","collinear_mass","Jet1_btag","Jet1_pt","TauJ_mass","mt"]: continue
