...
def GenTreeCode(tree, objlist, multithread=4):
    ...
    for wt_expr, wt_idx in indexed_wts.items():
        code.Add('double weightexpr_%i_ = %s;' % (wt_idx, wt_expr))
    for sel, objs in objs_by_sel.items():
        code.Add('if (%s) {' % sel)
        code.idlevel += 1
        for obj in objs:
            if multithread:
    ...
    if multithread:
        ...
        # Alternative, count the number of clusters (up to  a limit)
        code.Add('unsigned nc = 0; auto it = tree->GetClusterIterator(0); while ( it.Next() < tree->GetEntries() ) {++nc; if (nc >= 400) break;}')
        code.Add('if (nc >= 400) {')
        code.idlevel += 1
...
def DrawObjsHash(drawobjs):
    ...
    return hashlib.md5(str(allargs).encode('utf-8')).hexdigest()


def MultiDraw(node, samplesdir, sample_to_file_dict, tree_name, mt_cores=0, mt_thresh=1E7):
    ...
    # sys.exit(0)
    func_dict = {}
    for sample, drawobjs in drawtree.items():
        flatobjlist = []
        for objlist in drawobjs.values():
            flatobjlist.extend(objlist)

        print(sample_to_file_dict[sample])
        f = ROOT.TFile.Open(samplesdir+sample_to_file_dict[sample]+'.root')
        t = f.Get(tree_name)
...
def ReadTxtHistMCFM(filename, column=2, column_err=3, classname='TH1D', objname='hist', correct_density=False):
    with open(filename) as infile:
        lines = [tuple(line.split()) for line in infile.readlines() if not line.startswith('#')]
    N = (len(lines))
    edges = []
    ...
    errors = []
    for i in range(N):
        lo_edge = float(lines[i][0])
        hi_edge = float(lines[i][1])
        edges.append(lo_edge)
        if i == N - 1:
            edges.append(hi_edge)
        width = 1.0
        if correct_density:
            width = hi_edge - lo_edge
        contents.append(float(lines[i][column]) * width)
        errors.append(float(lines[i][column_err]))
    hist = getattr(ROOT, classname)(objname, objname, len(contents), array('d', edges))
    for i in range(N):
        hist.SetBinContent(i + 1, contents[i])
        hist.SetBinError(i + 1, errors[i])
