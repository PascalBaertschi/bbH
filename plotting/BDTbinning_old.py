...
def MakeStackedPlot(name, outdir, hists, cfg):
    ...
    if name=="BDToutSig":
      ...
      for i in range(Nbins):
        original_bins.append(round(hist_bbH.GetBinLowEdge(i+1),5))
      original_bins.append(1.)
      BinNumber = Nbins
      for i in range(Nbins):
        i = Nbins-i
        ...
        if bin_bbH>signal_integral/10.:
          print("sig error:",error_sig/bin_bbH)
          bins.insert(0,low_edge)
        else:
          print("remove:",low_edge)
          print("original_bins:",original_bins)
          original_bins.remove(low_edge)
          BinNumber -= 1
          hist_bbH = hist_bbH.Rebin(BinNumber,"hnew",np.array(original_bins))

        """
        hist_bbH = hist_bbH.Rebin(Nbins,"hnew",np.array(original_bins))
        #bbH_count += bin_bbH
        rebinned_bbH_bin = hist_bbH.GetBinContent(i)
        rebinned_bbH_error = hist_bbH.GetBinError(i)
        if rebinned_bbH_bin>signal_integral/10.:
          print("sig error:",rebinned_bbH_error/rebinned_bbH_bin)
          #bbH_count = 0.
          bins.insert(0,low_edge)
        else:
          print("remove:",low_edge)
        ...
        #print("bin:",bin_bbH," error:",error_sig)
        #print("bin_bkg:",bin_bkg)
        #print("error_bkg:",error_bkg)
        if bin_bkg < 0.: 
          bin_bkg = 0.
          error_bkg = 0.
        if bin_bbH < 0.: 
        ...
        bkg_count += bin_bkg
        #print("error_bkg:",error_bkg)
        error_sig_count = np.sqrt(error_sig_count**2+error_sig**2)
        error_bkg_count = np.sqrt(error_bkg_count**2+error_bkg**2)
        S_B_ratio = 0.
        
        if bbH_count==0 and bkg_count==0:
          S_B_ratio = 0.
        else:
          #S_B_ratio = bbH_count/bkg_count
          S_B_ratio = bbH_count/np.sqrt(bkg_count)
          #S_B_ratio = bbH_count/np.sqrt(bbH_count+bkg_count)
          #S_B_ratio = bbH_count/np.sqrt(bkg_count**2+error_bkg_count**2)
        if bin_bbH==0 and bin_bkg==0:
          ratio_per_bin = 0.
        else:
          #ratio_per_bin = bin_bbH/bin_bkg
          ratio_per_bin = bin_bbH/np.sqrt(bin_bkg)
          #ratio_per_bin = bin_bbH/np.sqrt(bin_bbH+bin_bkg)
          #ratio_per_bin = bin_bbH/np.sqrt(bin_bkg**2+error_bkg**2)
          #print("Bin",i,"from",low_edge,"to",high_edge,"ratio:",ratio_per_bin)
          #print("Bin",i,"from",low_edge,"to",high_edge,"bbH:",bin_bbH)
          #if S_B_ratio>integral_ratio/3:
        
        #if bbH_count>signal_integral/12. and error_bkg_count<0.9:
        #print("error_bkg_count:",error_bkg_count)
        #bkg_unw_eff = bkg_count**2/error_bkg_count**2
        #rel_error_bin = np.sqrt(bkg_unw_eff)/bkg_unw_eff
        sig_error = 0.
        if bin_bkg!=0.:
          bkg_error = error_bkg_count/bin_bkg
        if bin_bbH!=0.:
          sig_error = error_sig_count/bin_bbH
        #if bbH_count>signal_integral/12. and bkg_error<0.9:
          #print("cut:",signal_integral/12.)
        if bbH_count>signal_integral/10.:
          print("sig_error:",sig_error)
          bbH_count = 0.
          bkg_count = 0.
          error_bkg_count = 0.
          error_sig_count = 0.
        ...
        if bbH_count>signal_integral/10.  and bkg_error<0.9:
          #print("bbH_count:",bbH_count)
          #print("bkg_error:",bkg_error)
          print("sig_error:",sig_error)
          bbH_count = 0.
          bkg_count = 0.
          error_bkg_count = 0.
