...
def getLHEScaleSumw(dir,file,year,preVFP):
    path = dir+file+'__UL%s%s.json'%(year,preVFP)
    with open(path) as json_file:
        dict = json.load(json_file)
        dict_sumw = dict['LHEScaleSumw']
        return [dict_sumw['bin0'],dict_sumw['bin1'],dict_sumw['bin2'],dict_sumw['bin3'],dict_sumw['bin4'],dict_sumw['bin5'],dict_sumw['bin6'],dict_sumw['bin7'],dict_sumw['bin8']]

def getgenEventSumw(dir,file,year,preVFP):
    path = dir+file+'__UL%s%s.json'%(year,preVFP)
    with open(path) as json_file:
        dict = json.load(json_file)
        return dict['genEventSumw']
