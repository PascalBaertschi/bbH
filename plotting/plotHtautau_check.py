...
if isBDT:
    ...
    #samplesdir = '/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples_UL%s%s%s_oldTauID/'%(year,preVFP,sysvar)
...
ff_variations = ['ff%s_qcdUp'%year,'ff%s_qcdDown'%year,'ff%s_wUp'%year,'ff%s_wDown'%year,'ff%s_ttUp'%year,'ff%s_ttDown'%year,'ff%s_qcdfitpar1Up'%year,'ff%s_qcdfitpar1Down'%year,'ff%s_qcdfitpar2Up'%year,'ff%s_qcdfitpar2Down'%year,'ff%s_wfitpar1Up'%year,'ff%s_wfitpar1Down'%year,'ff%s_wfitpar2Up'%year,'ff%s_wfitpar2Down'%year,'ff%s_wfitpar3Up'%year,'ff%s_wfitpar3Down'%year,'ff%s_wfitpar4Up'%year,'ff%s_wfitpar4Down'%year,'ff%s_ttfitpar1Up'%year,'ff%s_ttfitpar1Down'%year,'ff%s_ttfitpar2Up'%year,'ff%s_ttfitpar2Down'%year]
...
if isBDT:
    drawvars = [    
        ...
        ('BDToutSig', 'BDToutSig',      [0,0.3,0.325,0.35,0.375,0.4,0.425,0.45,0.475,0.5,0.525,0.55,0.575,0.6,0.625,0.65,0.675,0.7,0.725,0.75,0.775,0.8,0.825,0.85,0.875,0.9,0.925,0.95,1]),
        #('BDToutSig', 'BDToutSig',      [0,0.3,0.4,0.45,0.5,0.525,0.55,0.575,0.6,0.625,0.65,0.675,0.7,0.725,0.75,0.775,0.8,0.825,0.85,0.875,0.9,0.925,0.95,1]),
    ...
    for sample in getsamples(channel,year,preVFP):
        if channel=="mutau":
            if sample=='data_obs': #no veto on gen matched taus and no selection on EventNumber
                selection = 'isHtoMuTau && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && nBjets_m>=1 && transverse_mass_lepmet<60'
                selection_ar = 'isHtoMuTauAR && dimuon_veto==0 && electron_veto==0 && Mu1_charge*Tau1_charge==-1 && extra_muon_veto==0 && nBjets_m>=1 && transverse_mass_lepmet<60'
            else:
                selection = 'isHtoMuTau && Mu1_charge*Tau1_charge==-1 && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Tau1_genmatch!=6 && nBjets_m>=1 && transverse_mass_lepmet<60 && EventNumber%10>=4'
                selection_ar = 'isHtoMuTauAR && dimuon_veto==0 && electron_veto==0 && extra_muon_veto==0 && Mu1_charge*Tau1_charge==-1 && Tau1_genmatch!=6 && nBjets_m>=1 && transverse_mass_lepmet<60 && EventNumber%10>=4'
        elif channel=="etau":
            if sample=='data_obs': #no veto on gen matched taus and no selection on EventNumber
                selection = 'isHtoETau && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && nBjets_m>=1 && transverse_mass_lepmet<60'
                selection_ar = 'isHtoETauAR && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Ele1_charge*Tau1_charge==-1 && nBjets_m>=1 && transverse_mass_lepmet<60'
            else:
                selection = 'isHtoETau && Ele1_charge*Tau1_charge==-1 && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Tau1_genmatch!=6 && nBjets_m>=1 && transverse_mass_lepmet<60 && EventNumber%10>=4'
                selection_ar = 'isHtoETauAR && dielectron_veto==0 && muon_veto==0 && extra_electron_veto==0 && Ele1_charge*Tau1_charge==-1 && Tau1_genmatch!=6 && nBjets_m>=1 && transverse_mass_lepmet<60 && EventNumber%10>=4'
        ...
        if sysvar=="":
            ...
            if isBDT:
                ...
                if sample!="data_obs":  #following uncertainties only for MC
                    ...
                    #if sample in ['bbH_htt','ggH_bb_htt_inc','ggH_bb_htt_excl','intH_bb_htt','intH_htt','VBF','ZH','ttH','bbH_nobb_htt','ggH_htt_inc','ggH_htt_excl']:
...
if isBDT and sysvar=="":
    ...
    if channel=="mutau":
        ...
        channel_sys = channel
    variations_list = ["","ff%sUp"%year,"ff%sDown"%year,"Hcut","Hsigcut","cut","sig"+year,"tt"+year,"dy"+year,"eff_trig_%s_%sUp"%(channel_sys,year),"eff_trig_%s_%sDown"%(channel_sys,year),"ttbarShapeUp","ttbarShapeDown","dyShapeUp","dyShapeDown","fake_m_%sUp"%year,"fake_m_%sDown"%year,"fake_e_%sUp"%year,"fake_e_%sDown"%year,"QCDscaleUp","QCDscaleDown","PS_ISRUp","PS_ISRDown","PS_FSRUp","PS_FSRDown"]+ff_variations+btag_variations+tauid_variations
elif sysvar!="":
    variations_list = [""]
else:
...
if isBDT:
    ...
    #filename = './root/%s_UL%s%s%s_oldTauID.root'%(channel,year,preVFP,sysvar_noyear)
...
if isBDT:
    if channel=="mutau":
        ...
        #filename_limit_comb = './root/htt_mt_bbH_comb.Run%s.root'%year
        ...
        #copyfile_to_comb = '/work/pbaertsc/bbh/CMSSW_10_2_13/src/CombineHarvester/bbHRun2Legacy/shapes/htt_mt_bbH_comb.Run%s.root'%year
        #hadd_rootfiles = "hadd -f ./root/htt_mt_bbH_comb.Run%s.root ./root/htt_mt_bbH.Run%s.root ./root/htt_mt_bbH_jesTotalUp.Run%s.root ./root/htt_mt_bbH_jesTotalDown.Run%s.root ./root/htt_mt_bbH_jerUp.Run%s.root ./root/htt_mt_bbH_jerDown.Run%s.root ./root/htt_mt_bbH_met_unclusteredUp.Run%s.root ./root/htt_mt_bbH_met_unclusteredDown.Run%s.root"%(year,year,year,year,year,year,year,year)
    elif channel=="etau":
        filename_limit = './root/htt_et_bbH%s.Run%s.root'%(sysvar_noyear,year)
        #filename_limit_comb = './root/htt_et_bbH_comb.Run%s.root'%year
        ...
        #copyfile_to_comb = '/work/pbaertsc/bbh/CMSSW_10_2_13/src/CombineHarvester/bbHRun2Legacy/shapes/htt_et_bbH_comb.Run%s.root'%year
        #hadd_rootfiles = "hadd -f ./root/htt_et_bbH_comb.Run%s.root ./root/htt_et_bbH.Run%s.root ./root/htt_et_bbH_jesTotalUp.Run%s.root ./root/htt_et_bbH_jesTotalDown.Run%s.root ./root/htt_et_bbH_jerUp.Run%s.root ./root/htt_et_bbH_jerDown.Run%s.root ./root/htt_et_bbH_met_unclusteredUp.Run%s.root ./root/htt_et_bbH_met_unclusteredDown.Run%s.root"%(year,year,year,year,year,year,year,year)
