...
def MakeStackedPlot(name, outdir, hists, Hbkg, cfg):
    if Hbkg=='VBF':
      color = TColor.GetColor(187, 5, 30)
    elif Hbkg=='ttH':
      color = TColor.GetColor(255, 0, 255)
    elif Hbkg=='ZH':
      color = TColor.GetColor(255, 163, 4)
    bkgscheme = [processComp(Hbkg,[Hbkg],color)]
    ...
    signal = [processCompSignal("bbH#tau#tau",["bbHtt"],4,1,1),processCompSignal("ggF (no b-jets)",["jjHtt"],4,2,1)]
...
parser.add_argument('--bkg', '-b', dest='Hbkg', type=str, default='ttH', action='store')
...
for path, subnode in node.ListNodes(withObjects=True):
  ...
  if ("Ele" in path and channel=="mutau") or ("Mu" in path and channel=="etau"):
    continue
  if path not in ["H_mass","BDToutSig","BDToutTT","BDToutDY","BDToutput"]: continue
  if "2D" in path:
  ...
  MakeStackedPlot(name, target_dir, hists, Hbkg, plotcfg)
