...
if __name__ == "__main__":
  ...
  parser.add_argument('-T', '--tes',     dest='tes', type=str, default="", action='store', choices=["","scale_t_1prongUp","scale_t_1prong1piUp","scale_t_3prongUp","scale_t_1prongDown","scale_t_1prong1piDown","scale_t_3prongDown"],
...
def getFileListLocal(year,sample,preVFP,JECvar,tes):
    """Get list of files from local directory."""
    if JECvar!="": #use preskimmed samples for JECvar
      filename = "./sampleslist/UL%s%s_skimJEC/%s.txt"%(year,preVFP,sample)
    elif tes!="": #use preskimmed samples for TES
      filename = "./sampleslist/UL%s%s_skimTES/%s.txt"%(year,preVFP,sample)
    else:
...
def main():
    ...
    if JECvar!="" or tes!="":
      batchSystem = 'slurm_runner_short.sh'
    ...
    for year in years:
      ...
      for channel in channels:
        ...
        for directory in os.listdir(samplesdir):
            ...
            name = directory
            files = getFileListLocal(year,directory,preVFP,JECvar,tes)
            if not files:
            ...
            #  continue
            if (JECvar!="" or tes!="") and directory.find("Run")>0: #run JEC variations only for MC samples
            ...
            if JECvar!="":
              ...
              if tes!="":
                jobList      = 'joblist/joblist_UL%s%s_%s_%s_%s%s.txt'%(year,preVFP,tes,name,channel,tag)
              else:
            ...
            jobs         = open(jobList,'w')
            #outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/%s/%s'%(year,name))
            if JECvar!="":
              outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/UL%s%s_%s/%s'%(year,preVFP,JECvar,name))
            else:
              if tes!="":
                outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/UL%s%s_%s/%s'%(year,preVFP,tes,name))
              else:
                outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/UL%s%s/%s'%(year,preVFP,name)) 
            #outdir       = ensureDirectory('/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/samples/%s%s%s/%s'%(ULtag,year,preVFP,name))
            #outdir       = ensureDirectory('/srm://t3se01.psi.ch:8443/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/%s/%s'%(year,name))
            #ensureDirectory(outdir+'/logs/')
            #outdir       = ensureDirectory('/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/pbaertsc/bbh/%s/%s'%(year,name))
            # NFILESPERJOBS
            nFilesPerJob = 1
            if nFilesPerJob<1:
